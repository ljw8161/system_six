...
int main(void)
{
	...
	{
		listener = socket(p->ai_family, p->ai_socktype, p->ai_protocol);	//채팅 참가자를 접소할 소켓을 개설(이 소켓을 초기소켓이라고 부르겠다)
		if (listener < 0)
		{
	...
	FD_SET(listener, &master);				// keep track of the biggest file descriptor	//*master 중 소켓 listener에 해당하는 비트를 1로 한다
	fdmax = listener; 						// so far, it's this one

	for(;;) {								// main loop
		read_fds = master; 					// copy it
		fflush(stdout);
		if (select(fdmax+1, &read_fds, NULL, NULL, NULL) == -1) {	//초기 소켓에 어떤 I/O 변화가 생길 때 까지 기다린다
			perror("select");
			exit(4);
		}
		flag = 0;
		for(i = 0; i <= fdmax; i++)		//fdmax == 접속자 수
		{			// run through the existing connections looking for data to read
			if (FD_ISSET(i, &read_fds))		//read_fds 구조체에서 소켓번호 i에서 I/O 변화가 있었으면 양수값인 소켓번호 s를 리턴한다
			{ 		// one connection
				if (i == listener)		//새 참가자 받기
				{			// handle new connections
					addrlen = sizeof remoteaddr;
					newfd = accept(listener,(struct sockaddr *)&remoteaddr,	&addrlen);	//새로운 참가자 접속 처리, 리턴하는 소켓번호를 참가자 리스트(newfd->master)에 등록
					if (newfd == -1) 
					{
						perror("accept");
					}
				...
				{								
					if ((nbytes = recv(i, buf, MAXDATASIZE, 0)) <= 0)	//채팅 메시지가 도착했는지 검사 -> 안함
					...
					{	
						...
						{								//enter the command code
							...
							{	//입력 초기 부분이 "/nick"일때 닉네임 바꾸기
								printf("Nickname has been changed to: ");
								for(k=6, j=0; k<strlen(buf); k++, j++)
								{			//transfer from buf to nickname
									nickname[j] = buf[k];
									if (k == (strlen(buf) - 1)) 
									{
										nickname[j + 1] = '\0';
								...
								if (send(arruser[i-4].socknum, msg, MAXDATASIZE , 0) == -1)		//arruser[i-4].socknum에게 msg를 보내기
								{
											perror("send");
								}
							}
							...
							{
								for (k = 8, j = 0; k < strlen(buf); k++, j++)
								{			//transfer from buf to uploadfile
									upfilename[j] = buf[k];
									if (k == (strlen(buf) - 1))
									{
										upfilename[j + 1] = '\0';
							...
							{		
								for(k=10, j=0; k<strlen(buf); k++, j++)
								{			//transfer from buf to downloadfile
							...
							if(strcmp("/ls", buf)==0)		//upload한 파일 목록 보기
							{
								buf[0] = '\0';
									for (filedir=opendir("upload"), filedirent=readdir(filedir); NULL != filedirent; filedirent=readdir(filedir))
									{
										strcat(buf, "\n");
