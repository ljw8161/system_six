...
int main(int argc, char *argv[])
{
	...
	if (connect(sockfd, servinfo->ai_addr, servinfo->ai_addrlen) == -1) {/*connect socket*/
		close(sockfd);
	...
	while(strcmp("/exit",buf)){/*exit chatting*/
		tv.tv_sec = 6000;
		tv.tv_usec = 500000;
		FD_ZERO(&readfds);/*disable all the socket numbers*/
		FD_SET(0,&readfds);/*file discriptor connected to server */
		...
		if (FD_ISSET(sockfd, &readfds)) { 		/*get message from server and print message*/	
		...
		else if (FD_ISSET(0, &readfds)) {/*send input to server*/
			...
			if(strncmp("/nick", msg, strlen("/nick"))==0){
				send(sockfd, msg, MAXDATASIZE, 0);/*to change nick name*/
			}
			else{
				send(sockfd, msg, MAXDATASIZE, 0);
			}
