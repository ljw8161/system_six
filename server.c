...
int main(void)
{
	...
	if ((rv = getaddrinfo(NULL, PORT, &hints, &ai)) != 0) {/*get socket address*/
		fprintf(stderr, "selectserver: %s\n", gai_strerror(rv));
		exit(1);
	}
	for(p = ai; p != NULL; p = p->ai_next) {
		listener = socket(p->ai_family, p->ai_socktype, p->ai_protocol);/*make socket*/
		if (listener < 0) {
			continue;						// lose the pesky "address already in use" error message
		}
		setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));/*set socket option*/
		if (bind(listener, p->ai_addr, p->ai_addrlen) < 0) {/*bind*/
		close(listener);
		continue;
		}
	...
	for(;;) {								// main loop
		...
		fflush(stdout);
		if (select(fdmax+1, &read_fds, NULL, NULL, NULL) == -1) {/*wait for next i/o */
			perror("select");
			exit(4);
		}
		flag = 0;
		for(i = 0; i <= fdmax; i++) {			// run through the existing connections looking for data to read
			if (FD_ISSET(i, &read_fds)) { 		// one connection
				if (i == listener) {			// handle new connections
					...
					newfd = accept(listener,(struct sockaddr *)&remoteaddr,	&addrlen);/*wait and accept*/
					if (newfd == -1) {
						perror("accept");
					}
					else 
