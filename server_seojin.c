...
int main(void)
{
	...
	FD_ZERO(&master);						//master소켓을 0으로 초기화 clear the master and temp sets
	FD_ZERO(&read_fds);						//read_fds 소켓을 0으로 초기화 get us a socket and bind it
	...
	for(p = ai; p != NULL; p = p->ai_next) {
		listener = socket(p->ai_family, p->ai_socktype, p->ai_protocol); //커널에 소켓 요청
		if (listener < 0) {
			continue;						// lose the pesky "address already in use" error message
		}
		setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));
		if (bind(listener, p->ai_addr, p->ai_addrlen) < 0) { //주소를 소켓에 bind.
		close(listener);
		continue;
		}
	...
	if (listen(listener, 10) == -1) { //소켓에서 queue size가 10인거 call queue size왜나와??
		perror("listen");
		exit(3);							// add the listener to the master set
	}
	FD_SET(listener, &master);				//초기 소켓 선택// keep track of the biggest file descriptor
	fdmax = listener; 						//fdmax~socket의 id값// so far, it's this one
	...
	for(;;) {								// main loop
		...
		for(i = 0; i <= fdmax; i++) {			// run through the existing connections looking for data to read
			if (FD_ISSET(i, &read_fds)) { 		// one connection
												//master에서 소켓 fd(j)에 해당하는 비트 set
												//FD_ISSET이 양수이면 클라이언트에서 연결요청(채팅참가요청)이 온것->accept() 호출해서 처리.
				if (i == listener) {			// handle new connections
					addrlen = sizeof remoteaddr;
					newfd = accept(listener,(struct sockaddr *)&remoteaddr,	&addrlen); //newfd에 새로운 소켓번호 추가해줬음->remoteaddr이라는 client리스트에 추가
					if (newfd == -1) {	
						perror("accept");
					}
					else //초기소켓이 아니면?
				...
				{								
					if ((nbytes = recv(i, buf, MAXDATASIZE, 0)) <= 0) {	// handle data from a client
						...
						if(buf[0] == '/'){								//enter the command code
							...
							strcat(msg, ">");							//>를 msg에 연결! 유저이름> 이렇게 뜨게끔
							strcat(msg, buf);							//buf를 msg에 연결  
							for(j = 0; j <= fdmax; j++) {				//모든 참가자에게 채팅 메시지 전송// send to everyone
